...
export default function useChart() {
  const makeRadarChart = (data: SkillItem[]) => {
    ...
    const areaColor = skillSets.filter((skillSet) => skillSet.category === skillCategory)[0]?.color ?? '#0593A2'

    return (
      <ResponsiveContainer width={400} height={300}>
        <RadarChart title='test' cx={200} cy={150} data={data} margin={{ top: 30, right: 50, bottom: 30, left: 50 }}>
          <text x={400 / 2} y={10} fill='#333' textAnchor='middle' dominantBaseline='central'>
          ...
          <PolarRadiusAxis
            ...
            angle={90}
          />
          <Radar name='name' dataKey='level' stroke={areaColor} fill={areaColor} fillOpacity={0.6} />
        </RadarChart>
      </ResponsiveContainer>
    )
  }
